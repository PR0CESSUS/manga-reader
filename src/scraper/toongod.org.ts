...
export class ToongodScraper extends Scraper {
  domain: string = "toongod.org";
  ...
  parseURL(url: string) {
    const match = url.match(/.*webtoon\/(.*?)\/(?:chapter-(\d+))?/);
    const slag = match[1];
    const episode = parseInt(match[2]) || 0;
    return { url: `https://www.toongod.org/webtoon/${slag}/`, episode: episode, genre: "", slag: slag };
  }
